...
#'@param sect Character. The country/countries you want to download variables for. Defaults to Australia.
...
if(!is.null(sect) & sect=='AUS'){data$variable <- data$variable %>% gsub("_AUSTRALI","",.)%>% gsub("_AUS_DET","",.)}
...
if(!is.null(sect) & sect=='AUS'){b$variable <- b$variable %>% gsub("_AUSTRALI","",.)%>% gsub("_AUS_DET","",.)}
rownames(b) <- c()
