...
ox_dated_bar_graph <- function(a,ttl,lh_units,x_range,y_range,x_break="1 year",srce="Source: Haver Analytics, BIS Oxford Economics",
                               ...
                               x_format="%Y",var_order=NULL,no_forc=0,date_adj=1,edit=0){
  ...
  if(second_axis==1){
    h <- h+ scale_y_continuous(breaks=seq(y_range[1],y_range[2],y_range[3]),limits=c(y_range[1],y_range[2]),expand=c(0,0),labels=comma,
  ...
  if(fc==1){
    h <- h+geom_vline(xintercept = as.numeric(as.Date(fc_date,"%d/%m/%Y")),size=1) +
      annotate("text",label="Forecast",y_range[2],x=as.Date(fc_date,"%d/%m/%Y"),size=20/2.83465,hjust=-0.05,vjust=1)}else
        if(fc==2){h <- h+geom_vline(xintercept = as.numeric(as.Date(fc_date,"%d/%m/%Y")),size=1) +
          annotate("label",label="Forecast",label.size=0,y_range[2],x=as.Date(fc_date,"%d/%m/%Y"),size=20/2.83465,hjust=0.5,vjust=0.5)
        }
  ...
  if(FY==1){
    h <- h+annotate("text",label="FY",y=y_range[1],x=x_range[2],size=20/2.83465,hjust=1,vjust=3.2)}
